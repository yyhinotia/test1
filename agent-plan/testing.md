...
## INC-TESTING-011：4v4 闭环、再战证据与人工验收剧本

- 状态：planned
- 创建时间：2026-09-25T20:47:54+08:00
- 最后修改：2026-09-25T20:47:54+08:00
- 主题：testing
- 目标：用真实主场景证明 2v2 → 3v3 → 4v4 的战斗、奖励、解锁、Build 重配与第二轮差异完整成立，并产出用户回答唯一问题所需的人工验收剧本与对照证据。
- 验收标准：
  - gameplay 用例加载真实 `main.tscn`，通过真实输入 / 按钮路由完成第一轮三段战斗，断言 2v2、3v3、4v4 的真实玩家与敌人单位数量、阵营和主选中单位状态。
  - 断言队友死亡不提前终局、主角死亡失败、敌人全灭胜利，以及多选移动 / 集火命令到达所有存活选中单位。
  - 首轮第三间通关后断言 `player_binding_skill.tres` 已解锁且灵石奖励不少于 45；不重复发放首次解锁。
  - 将 Build A（御剑斩 + 护体真气）切换为 Build B（御剑斩 + 定身术），断言只影响当前 Pawn 的运行时装配，随后再战一轮并记录生存、控制时长、战斗时长或承伤差异。
  - 产出 `build_decision_before_after` 对照表；即使自动化显示过程差异，也只能证明机制闭环，不能代替人工回答“是否愿意因为奖励重构并再战”。
  - 生成人工验收 5 步剧本：初始 Build 通关 → 看到奖励与解锁 → 选择是否重配 → 新 Build 再战 → 回答唯一问题；记录用户回答与结论。
  - 三档分辨率 1152x648 / 1152x720 / 800x720 的真实窗口截图与布局量测无重叠 / 无溢出；命令、退出码、关键输出和失败前后差异留档。
- 范围：`test/gameplay/` 下的真实主场景 Vertical Slice 闭环用例、必要的取证脚本与 `test/README.md` 索引、人工验收记录。
- 非范围：替代人工验收、正式平衡测试、随机化压力测试、AOE / 仇恨 / 存档 / 联网测试。
- 依赖：`INC-PAWNS-020`、`INC-PAWNS-021`、`INC-COMBAT-009`、`INC-WORLD-007`、`INC-CORE-012`、`INC-UI-018`；父 Increment `INC-CROSS-019`。
- 检索证据：2026-09-25T20:47:54+08:00 执行 `git status --short` 与 `git grep` 检查；`INC-TESTING-011` 未占用。现有 gameplay 证据覆盖单单位宗门闭环、秘境闭环与 Build 决策对照，但没有真实主场景 4v4 队伍、奖励解锁与再战闭环。
- 风险：真实主场景测试容易受暂停、自动隐藏与活动单位残留影响，必须在 `after_test()` 清理并优先读取公开 API；人工验收结论不能由自动化断言代替。已知 `main_scene_sect_test.gd` 324 orphans 与 `sect_panel_test.gd` 目录模式 288 orphans 是既有债务，需与本次新增失败区分。
- 实现说明：待实现。
- 变更文件：待实现。
- 测试证据：待实现。
- 验证状态：待验证
...
- 验收时间：待验收
- Git：待提交
- 备注：父 Increment `INC-CROSS-019` 的退出判定以本 Increment 的自动证据 + 用户人工回答共同组成；机制通过但用户回答“不会”时不得继续扩内容。
