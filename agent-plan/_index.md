...
> 最后修改：2026-09-26T13:46:30+08:00
> 规则来源：`../AGENTS.md`
...
| 战斗 | `combat.md` | 已验收 | `INC-COMBAT-013` | 2026-09-26T13:46:30+08:00 |
...
| 测试 | `testing.md` | 已验收 | `INC-TESTING-023` | 2026-09-26T13:46:30+08:00 |
...
| `INC-CROSS-025` | `accepted` | `INC-COMBAT-013`、`INC-TESTING-023` | 危险窗口的真代价（问题房间重新区分技能）：room 03 的答案从「护体真气硬吃」变回「定身打断」 | 通过（2/2 子项验证并验收：危险技能追加效果「护盾挡得住伤害、挡不住硬直」/ 问题房间重新定价「room 03 由 guard 变 binding，且仍不带打断可通关」；统一门禁 `RESULT: PASS`，GdUnit4 464 cases / 0 failures + headless 549 assertions） | 已验收 | 2026-09-26T13:46:30+08:00 |
...
| `INC-COMBAT-013` | `INC-CROSS-025` | combat | `accepted` | 危险技能的追加效果（护盾挡不住的硬吃代价） | 2026-09-26T13:46:30+08:00 | 验证通过（unit +1 用例（9 cases 全过）：`followup_skill` 可选且半配置按未配置处理；unit +1 用例（7 cases 全过）：聚煞术士的危险技能带硬直追加、Boss 不带；integration 机制事实：有盾硬吃 `blocked=true` + `skill_stunned` + 1.5833s 硬直，无盾硬吃同样，打断路径 0 次；聚煞术士危险窗口调度按实测修订为 2.5 / 1.2；统一门禁 `RESULT: PASS`） | 已验收 | `develop` / `0bb0ddb` |
| `INC-TESTING-023` | `INC-CROSS-025` | testing | `accepted` | 危险窗口追加效果证据与问题房间重新定价 | 2026-09-26T13:46:30+08:00 | 验证通过（integration +2 cases：护盾买不断硬直 / room 03 重新定价；改动前基线 room 03 = B 且窗口从未落地（3.02 开启 → 4.47 施法者死亡，无 hit/blocked），改动后 room 03 = A（打断 7.90s 余命 127 vs 硬吃 9.10s 余命 89，超出噪声）；事件时间线与窗口开启距离 74 < 射程 120 一并打印；统一门禁 `RESULT: PASS`，GdUnit4 464 cases / 0 failures + headless 549 assertions） | 已验收 | `develop` / `6f87700` |
...
| v5.54 | 2026-09-26T13:46:30+08:00 | 完成并验收 `INC-CROSS-025`（危险窗口的真代价）：复核六格矩阵后确认 room 03 与自述相反（说「定身打断是主要问题」，实测却是 `sword+guard` 更优），于是 `INC-COMBAT-013` 给危险技能加上「可选的追加效果」——`ActiveSkillDefinition.followup_skill` + `DangerWindowScheduler._apply_followup()`（只在释放路径、主效果之后结算）+ `EncounterSession` 记录 `skill_stunned`，聚煞术士接上新增的 `enemy_charge_hardstop.tres`（STUN / 1.6s / 零灵力零冷却，不是 `active_skills`，AI 不会主动施放），Boss 不配置、口径不变。实现过程中实测发现「只加追加效果」不够：施法者在 4.47s 被击杀，而危险技能要到 5.02s 才落地，窗口从未兑现（日志只有 `danger_window_opened`），因此如实把 `enemy_charge_adept` 的调度参数修订为 `interval 2.5 / duration 1.2`（窗口 2.52s 开启时玩家距施法者 74 < 定身射程 120，3.72s 落地），并把这条数值修订写进父级 Gate 4。`INC-TESTING-023` 补上护盾路径与时间线取证：有盾硬吃 `blocked=true` 仍记 `skill_stunned` + 1.5833s 硬直，打断路径 0 次；room 03 由 guard 变 binding（7.90s / 余命 127 vs 9.10s / 余命 89，超出噪声），且不带打断仍可通关。统一门禁 `RESULT: PASS`（GdUnit4 464 cases / 0 failures、headless 10 suites / 549 assertions / 0 failing suites，连续两次）。代码提交 `0bb0ddb` / `6f87700`，验收记录 `f59497c` / `8179de3` / 本提交。已知未解决：room 05「高防高血」仍打平、缺破防轴；若 Build 能在窗口落地前击杀施法者，这一层仍可被绕过。 |
...
## INC-CROSS-022：问题秘境接入游玩主路径（Build 观察闭环前置）

- 状态：accepted
- 创建时间：2026-09-26T12:35:49+08:00
- 最后修改：2026-09-26T12:50:34+08:00
- 目标：让 `problem_dungeon.tres`（11 个问题房间 + Boss、前 6 间挂首通技能奖励链）成为玩家在正常游玩（F5 / `main.tscn`）中默认可进入的秘境，同时保留试炼秘境作为可切换入口；回答 `INC-CROSS-021` Gate 3 的「遇到问题 → 获得技能 → 再遇到需要该技能的问题」在**游玩层**是否可达。
- 背景：`INC-CROSS-021` 验收后复核发现 `game/world/data/dungeons/problem_dungeon.tres` 只被数据 / 集成测试引用，`main.tscn` 的 `DungeonRun.default_dungeon` 仍是 `trial_dungeon.tres`（6 房间、无问题房间），`main.gd` 开机调用 `start_default_dungeon()`，因此正常游玩永远进不到问题秘境。该缺口此前已写入 `INC-WORLD-008` 的已知问题 ②，本次作为独立父 Increment 补完，不新增任何玩法内容或技能。
- 子 Increment：`INC-UI-020`（秘境选择区）、`INC-CORE-014`（主场景接线与默认入口）、`INC-TESTING-020`（游玩层可达性与首通奖励链证据）。
- 判定顺序：先 `INC-UI-020`（玩家能不能表达选择）→ 再 `INC-CORE-014`（默认入口与转发是否正确）→ 最后 `INC-TESTING-020`（游玩路径上是否真的成立）；三项全部验证通过后父级才进入 `awaiting_acceptance`。
- 验收标准（父级 Gate）：
  - Gate 1 默认可达：`main.tscn` 启动后 `DungeonRun` 运行的是 `problem_dungeon.tres` 的第 1 间房，面板读数与该秘境事实一致。
  - Gate 2 可切换且不丢内容：面板提供试炼 / 问题两个秘境选项，未进行中时可切换，切换后 `DungeonRun` 与面板读数同步；试炼秘境没有被从项目中移除。
  - Gate 3 锁定期：秘境进行中禁止切换，防止用切换绕过累积损耗，口径与既有「重新挑战锁定」一致。
  - Gate 4 奖励链在游玩路径可达：通过正式 `main.tscn` 路径清空问题秘境第 1 间后，该房间 `first_clear_skill_reward` 声明的技能进入玩家已解锁集合且不自动装配。
  - Gate 5 门禁：统一 `test/run_tests.ps1 -Layer all` → `RESULT: PASS`。
- 非范围：新增技能 / 敌人 / 房间、修改任何数值与伤害公式、随机掉落、装备与属性膨胀、存档迁移、正式美术与动画。
- 依赖：`INC-CROSS-021`（问题型敌人 / 问题房间 / 六格矩阵，已验收）；本父级不重开其结论。
- 风险：① 默认秘境从 trial 换成 problem 会触发一批「按 trial 硬编码」的主场景用例，必须按事实修正断言而不是放宽断言；② 若把选择入口做成不受 `is_active()` 约束，会重新引入战斗中满状态重开漏洞；③ 12 间问题秘境的整体节奏尚未人工体感评估，本父级只解决「能不能自然进入并拿到奖励」。
- 检索证据：2026-09-26T12:33+08:00 在仓库根目录执行 `git status --short --branch`（`develop...origin/develop` 一致，仅 `agent-plan` 有本次计划改动）、`git grep -h -o -E "INC-[A-Z]+-[0-9]{3}" -- agent-plan/`（UI 最大 019 / CORE 最大 013 / TESTING 最大 019 / CROSS 最大 021 / WORLD 最大 008，本批 `INC-UI-020` / `INC-CORE-014` / `INC-TESTING-020` / `INC-CROSS-022` 均未被占用）、`git grep -n "problem_dungeon\|trial_dungeon\|default_dungeon" -- game/ test/ tests/`（`problem_dungeon` 只出现在数据文件与测试；`game/main/main.tscn:18,71` 只有 trial；`game/main/main.gd:53` 开机走默认秘境）。
- 进度：2026-09-26T12:47:24+08:00 三个子 Increment 全部实现并验证通过（UI-020 9 cases、CORE-014 相关套件与门禁全绿、TESTING-020 2 cases），2026-09-26T12:49:30+08:00 用户以「本次验收通过」接受父级与三个子项（代码提交 `5138a41` / `54f1521` / `4dc4886`，验收记录 `c1b499d` / `4f10af8` / `7dabb45`）；本轮不改任何玩法数值。
- 用户验收：通过（用户原话「本次验收通过」；验收对象：INC-CROSS-022 三个子项）
...
- 进度：2026-09-26T13:46:30+08:00 完成并验收。两个子项都验证通过：`INC-COMBAT-013` 让危险技能可携带追加效果（聚煞术士的聚煞一击落地后追加 1.6s 硬直——伤害仍可被护盾吸收，硬直不能；Boss 不配置该字段），并按实测把聚煞术士的危险窗口调度修订为 `interval 2.5 / duration 1.2`（原 3.0 / 2.0 时施法者在窗口落地前就被击杀，房间自述的问题无法兑现）。`INC-TESTING-023` 给出机制证据（打断路径 0 次硬直、硬吃路径 1 次 + 1.5833s 硬直）与重新定价证据（room 03 由 guard 变 binding：打断 7.90s / 余命 127 vs 硬吃 9.10s / 余命 89，超出噪声），并保留「不带打断仍可通关」的负面对照。本批未新增技能 / 敌人 / 房间，只增加一种效果类型与一处已记录的调度参数修订。
- 用户验收：通过（用户原话「本次验收通过」；验收对象：INC-CROSS-025：危险窗口的真代价（问题房间重新区分技能））
- 验收时间：2026-09-26T13:46:30+08:00
- Git：`develop` / 子项代码提交 `0bb0ddb`（combat）、`6f87700`（testing），含 `f59497c`、`8179de3` 两条验收记录
