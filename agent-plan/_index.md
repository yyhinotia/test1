...
| `INC-CROSS-019` | `in_progress` | `INC-COMBAT-009`、`INC-PAWNS-021`、`INC-WORLD-007`、`INC-UI-018`、`INC-TESTING-011`、`INC-TESTING-012`（已实现前置：`INC-PAWNS-020` 多单位运行时；已退役：`INC-CORE-012`） | 1v1 → 1vN Build 玩法验证（Build Gameplay Validation） | 部分验证（Gate 0 机制层 + 1v1 首通奖励、Build 切换面板与跨遭遇延续） | 待验收 | 2026-09-25T22:30:00+08:00 |
...
- 实现说明：`INC-PAWNS-021`（最小运行时 Skill Loadout）、`INC-COMBAT-009`（1v1 危险技能窗口 + 轻量 CombatEvent）与 `INC-WORLD-007`（固定 1v1 / 1v2 / 1v3 试剑遭遇 + 首通定身术解锁 + 主动技能跨遭遇延续）均已实现并验证通过，等待用户验收；`INC-UI-018`（最小 Build A/B 切换面板）已实现并验证通过，`INC-TESTING-011` 待实现，`INC-TESTING-012`（`tests/` 场景入口）是人工 Gate 0 的前置。`INC-COMBAT-009` 同时把 `EncounterSession` 的终局判定落地为 1vN「敌方全灭才判胜、玩家单位死亡即失败」，为 Stage 2 的 1v2 / 1v3 解锁前提。设计基线 `docs/build-gameplay-validation.md` 随本批新增，`docs/4v4-vertical-slice.md` 标记为冻结延后。
