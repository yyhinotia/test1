...
- 最后修改：2026-09-25T21:14:43+08:00
- 主题：core
- 目标：把主场景的单一 `_selected_pawn` 选择模型升级为主选中单位加多选集合，并让移动 / 集火 / 暂停 / 施法输入按“队伍命令”和“主单位”正确路由。
- 验收标准：
  - 主场景维护 `_selected_pawns` 与 `_primary_selected_pawn`；左键选中、Shift 加入 / 移出、Ctrl+A / `select_all` 选中全部存活玩家单位、点击队伍卡切换主单位时两种状态一致。
  - 右键地面让所有选中存活单位按稳定偏移移动，右键敌方单位让所有选中存活单位集火同一目标；已死亡 / 已释放单位被安全忽略。
  - 空格仍切换暂停，暂停时允许观察、切换选中与下令；1~6 / Q 只作用于主选中单位，不把技能广播给整队。
  - `player_count` / 队伍成员由遭遇数据决定，主场景不硬编码 4 人；旧单人遭遇仍能选择并操作唯一玩家单位。
  - `_refresh_pawn_references()` 仍是唯一运行时引用刷新入口；换房 / 重开 / 单位死亡后选择集合不会保留失效实例。
  - 支持在同一 4v4 Boss 遭遇上重新开局 / 再战；Gate B 对照不要求重播 2v2 / 3v3 房间链，2v2 / 3v3 只服务 Gate A 技术验证。
  - 集成测试覆盖多选、移动、集火、死亡清理、暂停路由与单人兼容。
- 范围：`game/main/main.gd` 选择与命令路由、必要 InputMap / 主场景接线、相关 integration 测试。
- 非范围：拖拽框选、编队编号、单位朝向、路径寻路、技能队列、UI 卡片绘制、Build 重配面板绘制。
- 依赖：`INC-PAWNS-020`、`INC-COMBAT-009`；父 Increment `INC-CROSS-019`。
- 检索证据：2026-09-25T20:47:54+08:00 执行 `git status --short` 与 `git grep` 检查；`INC-CORE-012` 未占用。当前 `main.gd` 只有单 `_selected_pawn`，`_unhandled_input` 的鼠标命令与技能路由均按单单位实现。
- 风险：选择集合与主选中单位分裂会导致命令目标不一致；所有变更必须经统一选择辅助函数，并在单位死亡 / 场景重建时清理。暂停状态下的输入路由必须继续使用现有暂停机制，不能另开时间尺度。
- 实现说明：待实现。
- 变更文件：待实现。
- 测试证据：待实现。
- 验证状态：待验证
...
- 验收时间：待验收
- Git：待提交
- 备注：本 Increment 负责 Gate A 的多人命令路由，并支持 Gate B 在同一 4v4 Boss 上做 A/B 对照；四人 HUD 与 Build 预设选择由 `INC-UI-018` 负责。
