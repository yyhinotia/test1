...
- 验收标准：
  ...
  - Build 重配面板只在首通奖励可重配时出现，展示“御剑斩 + 护体真气”与“御剑斩 + 定身术”两套预设；选择后只调用 `Pawn.set_active_skill_loadout()`，失败时保持原状态并显示可复核的禁用 / 提示状态。
...
- 验收时间：待验收
- Git：待提交
- 备注：本 Increment 是 Build → Combat 闭环的可见操作面，不改变战斗和奖励规则。
