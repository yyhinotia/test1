...
## INC-UI-018：四人队伍 HUD 与 Build 重配面板

- 状态：planned
- 创建时间：2026-09-25T20:47:54+08:00
- 最后修改：2026-09-25T21:14:43+08:00
- 主题：ui
- 目标：提供不挤压现有左下 Dock 的四人队伍 HUD，并在首通奖励后提供两套预设 Build 的战术重配入口；UI 只发意图，不复制容量或技能规则。
- 验收标准：
  - 新增 `SquadPanel`，固定在屏幕底部中央；显示每名玩家成员的名称、生命比例、护盾 / 存活状态与主选中高亮，点击卡片只发出主选中意图。
  - 队伍卡数量由运行时队伍数据决定，2 / 3 / 4 人时布局正确；单位死亡后卡片显示死亡状态但不消失。
  - Build 重配面板只在首通奖励可重配时出现，展示“御剑斩 + 护体真气”与“御剑斩 + 定身术”两套预设；选择后只调用 `Pawn.set_active_skill_loadout()`，失败时保持原状态并显示可复核的禁用 / 提示状态。面板只由定身术解锁驱动，不展示或依赖 45 灵石，不诱导经济决策。
  - UI 不直接写 Pawn 技能数组，不复制境界容量表，不把技能可用性写死在面板。
  - 1152x648、1152x720、800x720 三档真实窗口截图与量测证明 SquadPanel / Build 面板不重叠、不溢出；`BottomLeftDock` 不被本次组件继续撑宽。
  - integration 测试覆盖四卡状态刷新、点击选中意图、两套预设与失败零副作用；取证脚本产出可复核量测。
- 范围：`game/ui/squad_panel.gd` / 场景资源（含 `.uid`）、Build 重配面板组件、主场景 UI 接线与三档分辨率取证脚本。
- 非范围：拖拽 Build 编辑器、技能图标正式美术、动画、敌人队伍卡、字体全面本地化。
- 依赖：`INC-CORE-012`、`INC-PAWNS-021`；父 Increment `INC-CROSS-019`。
- 检索证据：2026-09-25T20:47:54+08:00 执行 `git status --short` 与 `git grep` 检查；`INC-UI-018` 未占用。现有 HUD 只有单选中 Pawn 信息卡与技能栏，`BottomLeftDock` 已有 1152 逻辑宽度压力，因此新队伍面板必须独立放底部中央。
- 风险：底部中央面板可能遮挡战场点击或与现有 Dock 重叠；必须用真实窗口在窄屏验证交互矩形，并将非交互背景设为不拦截鼠标。另一个风险是 UI 直接复制容量逻辑，必须通过 Pawn API 裁决。
- 实现说明：待实现。
- 变更文件：待实现。
- 测试证据：待实现。
- 验证状态：待验证
...
- 验收时间：待验收
- Git：待提交
- 备注：本 Increment 是 Gate B1 的最小可见操作面，只做固定预设与卡片，不扩成 Build 编辑器；不改变战斗和奖励规则。
