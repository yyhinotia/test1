...
- 验收标准：
  - 新增 `game/ui/build/build_loadout_panel.tscn`（与配套 `.gd`），列出两套固定预设并标记当前 Build；点击「使用 Build B」等按钮只调用 `Pawn.set_active_skill_loadout()`。
  - 定身术未解锁前面板为锁定态、不可切换；解锁后由玩家主动切换。不得在首通时自动应用 Build B。
  - 切换失败时保持调用前状态，并在面板上显示可复核的失败状态，不静默吞掉错误。
  - 面板只读 `Pawn.get_known_active_skills()` / `get_equipped_active_skills()` 与容量 API，不直接读写 `PawnData.active_skills`，不硬编码技能可用性。
  - 面板不含拖拽、技能排序、多套保存、Build 命名、删除、分享等能力。
  - 切换成功后技能栏（`SkillBar`）与信息卡在同一帧跟随新装配。
  - 1152x648、1152x720、800x720 三档真实窗口截图与量测证明面板不与其他 HUD 元素重叠、不溢出，且非交互背景不拦截战场点击。
  - integration 测试覆盖：解锁前不可切换、解锁后主动切换成功、两套预设内容正确、失败零副作用、SkillBar 跟随。
- 范围：`game/ui/build/` 下的面板场景与脚本（含 `.uid`）、最小主场景接线、三档分辨率取证脚本、相关 integration 测试。
- 非范围：队伍 HUD 与 `SquadPanel`、拖拽 Build 编辑器、技能图标正式美术、动画、敌人队伍卡、字体全面本地化。
...
- 用户验收：通过（用户原话「本次验收通过」；验收对象：INC-UI-020：秘境选择区（只发信号的最小选项列表））
...
- 用户验收：通过（用户原话「本次验收通过」；验收对象：INC-UI-021：自定义 Build 编辑器（8 技能池 → 容量内自由组合））
...
- 用户验收：通过（用户原话「本次验收通过」；验收对象：INC-UI-022：首通解锁提示行（只读可见性））
