...
- 验收标准：
  ...
  - 第三间 Boss 首次通关固定解锁 `player_binding_skill.tres`（定身术），并给予至少 45 灵石；重复通关不重复发放首次解锁，灵石收益仍可正常入账。
...
- 风险：若把奖励直接写进 `main.gd`，后续无法区分“首通解锁”与“重复收益”；必须由秘境 / 宗门状态持有首通事实。另一个风险是队伍资源 UID 与成员引用不一致，需通过 Godot 加载测试验证资源图。
- 实现说明：待实现。
- 变更文件：待实现。
- 测试证据：待实现。
- 验证状态：待验证
...
- 验收时间：待验收
- Git：待提交
- 备注：本 Increment 只提供内容与奖励数据；多选、Build 面板和真实闭环证据分别由 `INC-CORE-012`、`INC-UI-018`、`INC-TESTING-011` 完成。
